...
namespace LiteNetLib
{
    public interface INatPunchListener
    {
		...
		void OnNatKeepAlive(string token);
    }
    ...
    public class EventBasedNatPunchListener : INatPunchListener
    {
		...
		public delegate void OnNatKeepAlive(string token);

        public event OnNatIntroductionRequest NatIntroductionRequest;
        public event OnNatIntroductionSuccess NatIntroductionSuccess;
		public event OnNatKeepAlive NatKeepAlive;
    ...
    public sealed class NatPunchModule
    {
		...
		private readonly Queue<KeepAliveEventData> _keepAliveEvents;
        ...
        internal NatPunchModule(NetManager netBase, NetSocket socket)
        {
			...
			_keepAliveEvents = new Queue<KeepAliveEventData>();
        }
        ...
        public void PollEvents()
        {
			...
			lock (_keepAliveEvents)
			{
				while (_keepAliveEvents.Count > 0)
				{
					var evt = _keepAliveEvents.Dequeue();
					_natPunchListener.OnNatKeepAlive(evt.Token);
				}
			}
        }
		...
		public void SendNatKeepAlive(NetEndPoint masterServerEndPoint, string additionalInfo)
		{
			//prepare outgoing data
			NetDataWriter dw = new NetDataWriter();
			dw.Put(additionalInfo, MaxTokenLength);
			...
			//prepare packet
			_netBase.SendRaw(NetPacket.CreateRawPacket(PacketProperty.NatKeepAlive, dw), masterServerEndPoint);
		}
		...
		private void HandleNatKeepAlive(NetDataReader dr)
		{
			string token = dr.GetString(MaxTokenLength);
			lock (_keepAliveEvents)
			{
				_keepAliveEvents.Enqueue(new KeepAliveEventData
				{
					Token = token
				});
			}
        ...
        internal void ProcessMessage(NetEndPoint senderEndPoint, PacketProperty property, byte[] data)
        {
            ...
            switch (property)
            {
                ...
                case PacketProperty.NatPunchMessage:
                    ...
                    break;
				case PacketProperty.NatKeepAlive:
					//We got punch and can connect
					HandleNatKeepAlive(dr);
					break;
            }
        }
    }
